...
Denon.prototype.getModelInfo = function (callback) {
    request.get('http://' + this.ip + this.status_url, {timeout: 1500}, function (error, response, body) {
        var xml = '';
...
Denon.prototype.setPowerState = function (powerState, callback) {
    ...
    request.get('http://' + this.ip + '/MainZone/index.put.asp?cmd0=PutZone_OnOff/' + powerState, {timeout: 1500},function (error, response, body) {
        if (!error && response.statusCode === 200) {
...
Denon.prototype.setMuteState = function(muteState, callback) {
    ...
    request.get('http://' + this.ip + '/MainZone/index.put.asp?cmd0=PutVolumeMute/' + muteState,{timeout: 1500}, function (error, response, body) {
        if(!error && response.statusCode === 200) {
...
Denon.prototype.setInput = function (input, callback) {
    request.get('http://' + this.ip + '/goform/formiPhoneAppDirect.xml?SI' + input, {timeout: 1500},function (error, response, body) {
        if (!error && response.statusCode === 200) {
            callback(null);
        } else {
            callback(error)
...
Denon.prototype.setVolume = function (volume, callback) {
    ...
    request.get('http://' + this.ip + '/goform/formiPhoneAppVolume.xml?1+' + vol,{timeout: 1500}, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            callback(null);
        } else {
            callback(error)
